...
if __name__ == '__main__':
    ...
    turbineY_free = np.array([0.,10000000000.])
    ...
    for i in range(len(angles)):
        ...
        speeds, _ = get_speeds(turbineX, turbineY_free, x_locs, y_locs, z_locs, wind_speed,shearExp=shear_free)
        ...
        speeds, _ = get_speeds(turbineX, turbineY_waked, x_locs, y_locs, z_locs, wind_speed,shearExp=shear_wake)
    ...
    # f_atm_free = interp1d(t[star:-1], atm_free, kind='cubic')
    ...
    # plt.plot(t,M_free[star:-1],'--k', label='CCBlade')
    # plt.plot(t,FAST_free[star:-1],'-k', label='FAST')
    # plt.plot(t,M_free[star:-1],'--r')
    # plt.plot(t,FAST_free[star:-1],'-r')
    # plt.plot(t,atm_free,'-r', label='freestream')
    #
    # plt.plot(t,M_waked[star:-1],'--b')
    # plt.plot(t,FAST_waked[star:-1],'-b')
    ...
    for k in range(len(delt)):
        ...
        for i in range(len(angles)):
            ...
            speeds, wake_radius = get_speeds(turbineX_actual, turbineY_actual, x_locs, y_locs, z_locs, wind_speed,shearExp=shear_actual)

            if i == 0:
                actual_speed = get_eff_turbine_speeds(turbineX_actual, turbineY_actual, wind_speed)[1]
                Omega_actual = (Omega_waked + (Omega_free-Omega_waked)/(free_speed-waked_speed) * (actual_speed-waked_speed))*4.9/6.
        ...
        # for i in range(5):
            ...
            M_actual_turb[i] = f_actual(pos_actual) + f_atm_waked(t[i])
