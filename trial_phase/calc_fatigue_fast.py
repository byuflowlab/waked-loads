...
"""not used"""
def extract_moments(filename_free,filename_close,filename_far):
    ...
    # ccblade_free, _ = calc_moment(speeds,Rhub,r,chord,theta,af,Rhub,Rtip,B,rho,mu,precone,hubHt,nSector,Omega_free,pitch,azimuth=az)
    #
    # #waked close
    ...
    # ccblade_close, _ = calc_moment(speeds,Rhub,r,chord,theta,af,Rhub,Rtip,B,rho,mu,precone,hubHt,nSector,Omega_close,pitch,azimuth=az)
    #
    # #waked far
    ...
    # ccblade_far, _ = calc_moment(speeds,Rhub,r,chord,theta,af,Rhub,Rtip,B,rho,mu,precone,hubHt,nSector,Omega_far,pitch,azimuth=az)

    # return atm_free-ccblade_free,atm_close-ccblade_close,atm_far-ccblade_far
    return atm_free,atm_close,atm_far
...
def extract_damage(filename_free,filename_close,filename_far):
    ...
    damage_free = get_damage(atm_free,freq,fos=3)
    damage_close = get_damage(atm_close,freq,fos=3)
    damage_far = get_damage(atm_far,freq,fos=3)
...
def calc_damage(turbineX,turbineY,turb_index,freq,atm_free,atm_close,atm_far,wind_speed=8.,rotor_diameter=126.4,TI=0.11):
    ...
    damage_4 = get_damage(atm_close,freq,fos=3)
    damage_10 = get_damage(atm_far,freq,fos=3)
    damage_free = get_damage(atm_free,freq,fos=3)
...
def farm_damage(turbineX,turbineY,windDirections,windFrequencies,atm_free,atm_close,atm_far):
    """
    ...
    for j in range(nDirections):
            ...
            for i in range(nTurbines):
                    damage[i] += calc_damage(turbineXw,turbineYw,i,windFrequencies[j],atm_free,atm_close,atm_far)
    return damage
